...
public class WeightChartModel {
    ...
    public WeightChartModel(Context context) {
    ...
    public void parse(List<UserWeight> weightRecords, int metric) {
        ...
        if (weightRecords.size() != 0) {
            this.mStartTime = weightRecords.get(0).getTime() - this.START_TIME_OFFSET;
            this.minDayOffset = 0;
            endTime = weightRecords.get(weightRecords.size() - 1).getTime() + this.START_TIME_OFFSET;
            if (weightRecords.get(weightRecords.size() - 1).getTime() < Calendar.getInstance().getTimeInMillis()) {
                endTime = Calendar.getInstance().getTimeInMillis() + this.START_TIME_OFFSET;
            }
            for (UserWeight wr : weightRecords) {
                if (wr.getWeight() < ((float) this.minWeight)) {
                    this.minWeight = (int) wr.getWeight();
                }
                if (wr.getWeight() > ((float) this.maxWeight)) {
                    this.maxWeight = (int) wr.getWeight();
                }
                this.mWeightChartRecords.add(new WeightChartRecord(wr.getWeight(), (int) ((wr.getTime() - this.mStartTime) / DAY_TIME)));
            }
            /*  for (UserWeight wr : weightRecords) {
    ...
    private List<WeightChartMonthLine> parseMonthLimitLines(long start, long end) {
        ...
        while (calendar.getTimeInMillis() <= end) {
            ...
            calendar.add(DAY_OF_MONTH, DAY_OF_MONTH);
        }
        return mMonthLimits;
    }
